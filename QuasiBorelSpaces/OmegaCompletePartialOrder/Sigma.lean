...
instance : OmegaCompletePartialOrder ((i : I) × P i) where
  ωSup c :=
    let c' := Chain.Sigma.distrib c
    ⟨c'.fst, ωSup c'.snd⟩
  le_ωSup c i := by
    simp only [ge_iff_le]
    cases c using Chain.Sigma.distrib_cases with
    | mk c =>
      simp only [Chain.Sigma.inj_coe, Chain.Sigma.distrib_inj, Sigma.mk_le_mk_iff]
      apply le_ωSup
  ωSup_le := by
    rintro c ⟨x, y⟩ h
    simp only [ge_iff_le]
    ...
    | mk c =>
      ...
      have h' := (h 0).choose
      subst h'
      simp only [exists_const, Chain.Sigma.distrib_inj, Sigma.mk_le_mk_iff, ωSup_le_iff] at ⊢ h
      exact h
